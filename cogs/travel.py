...
class TravelCog(commands.Cog, name="Travel"):
    ...
    async def travel(self, ctx, *, city_name: str = None):
        ...
        async def _out(content: str | None = None, *, embed: discord.Embed | None = None, view=None):
            # When invoked from a button/select, edit the current message to avoid spam.
            if component_interaction:
                await component_interaction.message.edit(content=content, embed=embed, view=view)
                return
            await ctx.send(content, embed=embed, view=view)

        if not player:
            return await _out(f"{ctx.author.mention} 尚未踏入修仙之路，请先使用 `{COMMAND_PREFIX}创建角色`。")
        if player["is_dead"]:
            return await _out(f"{ctx.author.mention} 道友已坐化。")
        ...
        if not arg:
            ...
            return await _out(ctx.author.mention, embed=embed, view=TravelRegionView(ctx.author, menu_cog))
        ...
        if arg in regions:
            ...
            return await _out(ctx.author.mention, embed=embed, view=TravelCityView(ctx.author, menu_cog, cities))
        ...
        if arg == "秘地":
            ...
            return await _out(ctx.author.mention, embed=embed, view=TravelSecretView(ctx.author, menu_cog, player_idx))

        import time
        now = time.time()
        if player["cultivating_until"] and now < player["cultivating_until"]:
            remaining = seconds_to_years(player["cultivating_until"] - now)
            return await _out(f"{ctx.author.mention} 道友正在闭关，无法移动。还剩约 **{remaining:.1f} 年**，可使用 `{COMMAND_PREFIX}停止` 提前结束。")
        if player["gathering_until"] and now < player["gathering_until"]:
            remaining = seconds_to_years(player["gathering_until"] - now)
            return await _out(f"{ctx.author.mention} 道友正在采集中，无法移动。还剩约 **{remaining:.1f} 年**。")
        ...
        if not target:
            ...
            if secret:
                ...
                if player_idx < req_idx:
                    return await _out(f"{ctx.author.mention} 境界不足，前往 **{secret['name']}** 需达到 **{secret['min_realm']}**。")
                ...
                if len(matches) == 1:
                    ...
                    return await _out(f"{ctx.author.mention} 找到多个匹配城市：{names}，请输入完整城市名。")
                else:
                    return await _out(f"{ctx.author.mention} 未找到城市「{arg}」，请检查名称是否正确。")
        ...
        view = None
        if component_interaction:
            from utils.views.travel import TravelAfterMoveView
            view = TravelAfterMoveView(ctx.author, menu_cog)
        await _out(ctx.author.mention, embed=embed, view=view)
